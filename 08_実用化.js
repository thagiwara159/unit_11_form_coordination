...
/** 
 * .getDataRange().getValue()で各シートの全データ取得できる
 * ドライブにファイル保存はDriveApp.createFile(ファイル名，タイムスタンプなど)
 * データをJSON形式に変換　　JSON.stringify()
 * .getUi().alert(コメント,ファイル名など)でユーザーに見える形でお知らせ
 * has は「～があるか？」で使われる接頭語 所有/存在
 * is は「～であるか？」を表す真偽値用の接頭語 状態/性質
 * can は「～できるか？」を表す接頭語　可能/権限
*/